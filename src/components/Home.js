...
class Home extends Component {
  ...
  componentDidMount() {
    axios.get('https://api.quotable.io/random').then((resp) => {
      ...
      setTimeout(() => {
        this.setState({
          quote: 'I built this place to help inspire young ninjas',
          author: Date().slice(0, 16),
        });
      }, 2000);
      setTimeout(() => {
      ...
      setTimeout(() => {
        this.setState({
          quote: 'Check out this motivational quote and try to stay motivated!',
          author: Date().slice(0, 16),
        });
      }, 8000);
      setTimeout(() => {
    ...
    return (
      <main className="bg-img pt-32 min-w-screen bg-cover">
        <section className="relative justify-center h-full lg:px-8 lg:mx-20">
          <div className="h-auto grid p overflow-auto min-w-screen wrapper hover:bg-gray-500 mx-4 rounded bg-red-400 shadow-2xl bg-opacity-100 lg:rounded-full">
